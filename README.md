...
**问题修复**:
- **文件**: `backend/app/services/estimation_api_client.py`
- **问题**: 组合收益率图表中当日收益额显示异常（显示为 ¥0.00）
- **原因**: `get_batch_estimations` 方法中单个基金估值获取异常处理不完善
- **修复**: 添加空数据处理分支，改进异常日志输出，确保单个基金获取失败不影响整体批处理

**⚠️ 已知问题**: 收益曲线计算正确性尚未完全验证，正在持续调试中
